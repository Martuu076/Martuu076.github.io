...
class Catalogo:
    ...
    def __init__(self, host, user, password, database):
        ...
        self.cursor.execute('''CREATE TABLE gatitos
                            (id_gatito INT NOT NULL AUTO_INCREMENT ,
                            nombre VARCHAR(50) NOT NULL ,
                            descripcion VARCHAR(255) NOT NULL ,
                            imagen_url VARCHAR(255) NOT NULL ,
                            PRIMARY KEY (id_gatito))''')
        self.conn.commit()
    ...
    def mostrar_gatito(self, id_gatito):
        ...
        # if gatito:
            ...
            # print(f"Imagen.....: {gatito['image_url']}")
            # print("-" * 40)
        # else:
        #     print("Gatito no encontrado.")

    def agregar_gatito(self,nombre,descripcion,image_url):
        sql = "INSERT INTO gatitos (nombre, descripcion, image_url) VALUES (%s, %s, %s)"
        valores = (nombre,descripcion,image_url)

        self.cursor.execute(sql,valores)
        self.conn.commit()
        return self.cursor.lastrowid

    def modificar_gatito(self, id_gatito, nueva_nombre, nueva_descripcion, nueva_image_url):
        sql = "UPDATE gatitos SET nombre = %s, descripcion = %s,  image_url = %s, WHERE gatitos = %s"
        valores = (nueva_nombre,nueva_descripcion,nueva_image_url, id_gatito)
    ...
    def eliminar_gatito(self, id_gatito):
        ...
        self.cursor.execute(f"DELETE FROM gatitos WHERE gatitos = {id_gatito}")
        self.conn.commit()
        return self.cursor.rowcount > 0
...
catalogo = Catalogo(host='saporapo.mysql.pythonanywhere-services.com',user='saporapo',password='Miyuki1103',database='saporapo$garritas')

ruta_destino = '/home/saporapo/mysite/static/imagenes'

@app.route("/gatitos", methods=["GET"])
def listar_gatitos():
    gatito = catalogo.listar_gatitos()
...
# Se llama al método modificar_producto pasando el codigo del producto y los nuevos datos.
    ...
    if 'imagen' in request.files:
            ...
            if gatito: # Si existe el producto...
                imagen_vieja = gatito["imagen_url"]
        ...
        gatito = catalogo.consultar_gatito(id_gatito)
        if gatito:
            nombre_imagen = gatito["imagen_url"]
...
def eliminar_gatito(id_gatito):
    ...
    if gatito:
        ...
        ruta_imagen = os.path.join(ruta_destino, gatito['imagen_url'])
        if os.path.exists(ruta_imagen):
            os.remove(ruta_imagen)
